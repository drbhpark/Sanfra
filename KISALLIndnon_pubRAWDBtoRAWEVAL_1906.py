...
def moddata(row_code):            # 라인별 임포트 모듈
    # temp=df.ix[row_code]
    temp = df.loc[row_code, '2013':'2018']
...
if __name__ == "__main__":
    ...
    PPL = DBlist('C:/a/ext1905byKISticker6.db') # 리스트 호출~ 갱신시 여기 바꿔주고
    temp_df = {}

    conn = sqlite3.connect("c:/a/evalrawkissIP1905_6yrs.db")  # 새로운 db생성 - 갱신시 여기 바꿔주고

    for k, l in enumerate(PPL):
        try :
            df = import_com(l,'C:/a/ext1905byKISticker6.db')    # 데이터 처리하여 DF로 리턴 - 갱신시 여기 바꿔주고
            #code = df['2018'][1]
            #sales2017 = df['2017'][3]
