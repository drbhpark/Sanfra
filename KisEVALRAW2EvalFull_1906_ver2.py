    #12월3일현재 잘 작동하는 공식버전. kis 로부터 받은 evalrawkissIP1905_6yrs를 받아서 변환.
#
# 싫행성공/ 19-6-03
...
def import_com(com_code, fileloc):
    ...
    df1 = df[['2013', '2014', '2015', '2016', '2017', '2018']]
    return df1
...
# 항목매칭작업
    temp = {
        ...
        '34 Number of Employees' : "N_Employees",
...
def build_FS(data1):
    ...
    data = data1.rename(columns={'2013': 'B', '2014': 'C', '2015': 'D', '2016':'E', '2017': 'F', '2018': 'G'})  # 인덱스 변경
...
if __name__ == "__main__":
    ...
    PPL = DBlist('C:/a/evalrawkissIP1905_6yrs.db') # 리스트 호출~
    PPL.sort()
    conn = sqlite3.connect("c:/a/evalKisNonPubDATA1906.db")  # 새로운 db생성
    #temp_df = {}

    for k, l in enumerate(PPL):
        raw = import_com(l,'C:/a/evalrawkissIP1905_6yrs.db')     # 데이터 처리하여 DF로 리턴
        df1 = reindex(raw)
        ...
        CR = CR.round(3)
        CF = CF.drop("G",axis = 1)
        CF = CF.rename(index=str, columns={"B": "C", "C": "D", "D": "E", "E": "F", "F": "G"}) #eval에는 한칸밀려서 있다. 그걸 한칸 밀어주는 작업
        Wholedata = pd.concat([wholeFS, CF, RA, CR], axis=0)
        Wholedata = Wholedata.drop(["B"], axis=1)
        WholedataN = Wholedata.rename(index=str, columns={"C": "2014", "D": "2015", "E": "2016", "F": "2017","G":"2018"})
